...
describe("ReviewWorkspace", () => {
  ...
  it("connects the risk queue, requirement detail, and PDF evidence columns", async () => {
    ...
    expect(screen.getByRole("button", { name: /GRI 2-1-a/ })).toHaveAttribute("aria-current", "true");
    ...
    expect(screen.getByRole("button", { name: "证据" })).toHaveAttribute("aria-pressed", "true");
    expect(screen.getByTitle("PDF 证据")).toHaveAttribute("src", expect.stringContaining("/pages/6/image"));
  });
  ...
  it("shows detail loading and error states after selecting a queue item", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn((input: string | URL | Request) => {
      const url = String(input);
      if (url.includes("/assessments/a-1")) return Promise.resolve(response(detail));
      if (url.includes("/assessments/a-2")) return detailPromise;
      if (url.endsWith("/dashboard")) return Promise.resolve(response({ high_priority_total: 2, high_priority_reviewed: 0 }));
      if (url.endsWith("/api/reports/report-1")) return Promise.resolve(response({ page_count: 78 }));
      return Promise.resolve(response(queue));
    }));
