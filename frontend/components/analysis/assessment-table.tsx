...
import { riskLabels, verdictLabels } from "@/lib/business-labels";
  ...
  return (
    <div>
      ...
      <div className="mt-4 overflow-x-auto border-y border-border">
        <table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-muted text-xs text-muted-foreground"><tr><th className="px-3 py-2">Requirement</th><th className="px-3 py-2">主题</th><th className="px-3 py-2">当前结论</th><th className="px-3 py-2">风险</th><th className="px-3 py-2">复核状态</th><th className="px-3 py-2">证据页</th></tr></thead><tbody className="divide-y divide-border">{query.data.items.map((item) => <tr key={item.assessment_id}><td className="px-3 py-3 font-medium">{item.requirement_id}</td><td className="px-3 py-3">{item.gri_topic}</td><td className="px-3 py-3">{verdictLabels[item.effective_verdict] ?? item.effective_verdict}</td><td className="px-3 py-3">{riskLabels[item.risk_level] ?? item.risk_level}</td><td className="px-3 py-3">{item.review_status}</td><td className="px-3 py-3">{item.source_pdf_pages.join(", ") || "-"}</td></tr>)}</tbody></table>
      </div>
    </div>
  );
}
