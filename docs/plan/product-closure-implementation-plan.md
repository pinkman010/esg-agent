...
> **执行要求：** 本计划只有在 `docs/plan/product-closure-realignment-plan.md` 的设计审批清单获得人工批准后才能执行。执行时使用 `executing-plans`，保持纵向切片、TDD 和小提交。

**目标：** 在现有 Next.js/FastAPI 单报告分析基础上，交付报告管理、577 条后台分析进度、固定风险队列、追加式人工复核、三栏工作台、整改任务和版本化输出。

**架构：** 系统 assessment/evidence 保持不可变；人工结果、风险、阶段、整改和输出使用独立表。每个切片同时交付数据库、后端 API、前端页面和测试，OpenAPI 是前端类型唯一来源。
...
一次只执行一个切片。每个切片通过测试、OpenAPI 类型生成和人工界面检查后再进入下一切片。
...
每个切片提交前人工确认对应界面或 API 行为。以下情况立即停止：数据库迁移会丢失现有人工记录；正式输出无法追溯 snapshot；普通界面需要暴露内部 route/profile 才能工作；577 分析出现非预期回归；需要引入多租户或新技术栈。
