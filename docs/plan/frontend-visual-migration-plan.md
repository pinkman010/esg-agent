...
| 阶段 4：主观视觉确认 | 完成 | 2026-08-24 已完成桌面与 390px 窄屏自动浏览器复验，用户随后确认本轮验收收尾。 |

受控调整：首次无报告场景保留带上传入口的业务引导 hero，未替换为通用 `EmptyState`；`EmptyState` 组件继续服务列表或筛选空态。该调整保留了首屏任务引导，不改变业务状态语义。

完整工程验收见 `docs/product/frontend-visual-migration-acceptance.md`。本轮视觉基线已冻结；后续视觉迭代应以实际使用问题为触发条件，避免继续扩大当前发布范围。
...
本轮未增加资产，未修改后端、API、数据库或业务语义。用户已确认本轮验收收尾，该实现纳入 `v1.3.1` 后续前端冻结基线；正式版本号与 tag 仍保持 `v1.3.1`，后续发布单独执行。

## 8. ESG 报告入口主图与低质量 OCR 证据降级（2026-08-24）

`/reports` 已纳入共享主图基线，复用 `ReportPageHero` 与 `module-policy-disclosure.webp`。主图静态、无遮罩、opacity 为 0.23 并使用 eager 加载；原报告列表、上传面板、数据请求和响应式单列布局保持不变。

人工复核页同时利用现有 `source_method` 和 `quality_flags` 完成展示层降级：`pdfplumber` 证据标记为“PDF 原文”并直接显示；带 `needs_manual_review` 的 OCR 证据显示质量警告，原始识别文本默认折叠，用户仍可展开审计并定位 PDF 原页。该调整没有新增后端字段，也没有更改证据、规则或人工结论。

完整前端门禁为 42 个测试文件、158 项测试通过，typecheck 和 production build 通过；lint 为 0 error、2 个既有 warning。桌面 1280×720 与移动端 390×844 浏览器验收确认两页均无横向溢出，OCR 原文默认隐藏、展开可审计，移动端证据切换可定位 PDF 第 77 页。控制台没有本轮新增 error；侧边栏图片的 LCP 与 `fill + sticky` 提示为既有开发模式告警。

本轮只修改前端展示、测试和文档，未修改后端、数据库、API、GRI、AI、RAG、OCR 执行链路、人工快照或导出语义。用户已确认验收收尾，当前视觉与 OCR 展示基线冻结；下一阶段停止扩展视觉范围，回到真实使用与问题收集。
