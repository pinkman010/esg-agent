...
轻度清晰化完成后，用户反馈侧边栏和主图仍不够清楚，首页横向主图最明显。用户批准移除全部遮罩，并继续使用图片自身 opacity 控制清晰度。

### 6.2 实施边界

- 删除桌面侧边栏的白色线性渐变遮罩和绿色径向光晕遮罩。
- 删除工作台首次使用、工作台已有报告和报告总览主图的线性渐变遮罩。
- 保留侧边栏 `opacity-50`，工作台首次使用 `opacity-[0.28]`、已有报告 `opacity-[0.25]`、报告总览 `opacity-[0.23]`。
- 保留图片源文件、object position、brightness、saturate、contrast 和 Ken Burns 动效。
- 不修改导航、文字层、按钮、状态徽章、布局、API、后端或业务语义。

### 6.3 测试设计

1. 先把四种状态的测试从渐变参数断言改为“图片 opacity 保持、图片后不存在遮罩元素”，确认旧实现因仍有遮罩而失败。
2. 删除五个遮罩元素后重新运行 3 个目标测试文件，确认 9 项测试通过。
3. 浏览器分别核查桌面工作台、报告总览和 390px 窄屏；重点观察侧边栏导航、首页标题和按钮、报告总览状态徽章的对比度。
4. 运行完整 `pnpm test`、`pnpm lint`、`pnpm typecheck` 和 `pnpm build`。

### 6.4 终止条件

- 本轮只删除遮罩，不继续提高图片 opacity。
- 若真实页面文字对比度不足，停止继续增强；后续优先降低对应图片 opacity，不恢复大面积白色遮罩。
- 自动验收通过后仍由用户做主观清晰度确认，再决定是否形成补丁版本。

### 6.5 执行结果

- 已删除桌面侧边栏 2 个遮罩和 3 个横向主图遮罩，共 5 个遮罩元素；四种图片 opacity 保持不变。
- 测试先观察到 4 个遮罩缺失断言按预期失败；删除遮罩后 3 个目标测试文件、9 项测试通过。
- 桌面浏览器核查确认首页横向图、侧边栏和报告总览主图明显增强，标题、按钮、`577`、导航和状态徽章仍可读。
- 390px 窄屏仍隐藏桌面侧边栏并显示移动端主导航，布局没有变化。
- 完整前端门禁通过：39 个测试文件、150 项测试，typecheck 和 production build 通过；lint 保持 0 error、2 个既有 warning。
