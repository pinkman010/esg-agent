...
**目标：** 将当前混合“披露结论、证据质量和复核优先级”的 `risk-v1` 调整为可解释的 `risk-v2.1`，使 `unknown` 不再天然升级为高优先级或批量占据中优先级；同时修复 577 条核查表和高优先级复核队列只能访问第一页的问题，并消除产品页面及导出中的生成式英文模板。
...
产品界面统一使用“复核优先级”。数据库和旧 API 中的 `risk_level` 暂时保留，避免破坏现有调用者；`risk-v2.1` 下该字段的业务含义等同 `review_priority`。
...
以下项目在没有明确异常或冲突时进入低优先级：

- `unknown` 且没有有效证据；该结论只表示人工复核顺序靠后，披露结论仍为 `unknown`，适用性仍为待判定；
- `partial` 且存在质量正常的直接实质证据；
- `disclosed` 且存在质量正常的直接实质证据；
- 人工确认不适用的项目。

#### 规则限制

- `unknown_verdict` 可以保留为解释原因，但不能单独触发高优先级或中优先级；
...
- 适用性待判定项目进入独立工作队列；该队列不改变复核优先级，且必须支持批量或主题级适用性确认，避免逐条处理 343 项；
- 当前远景 577 条验收基线预计为高 12、中 60、低 505，另有适用性待判定 343 条。数量仅用于冻结本次输入数据的回归结果，不进入分类规则或生产配置。
...
- [x] **步骤 1：冻结 risk-v1 行为**

保留现有 `risk-v1` 单元测试，明确断言其历史行为不随新分类器改变。测试使用显式规则版本，避免默认版本升级后误测。

- [x] **步骤 2：写 risk-v2 规则矩阵失败测试**

至少覆盖：

```text
unknown + no evidence → low + applicability undetermined
unknown + index/omission only → medium
partial + valid direct evidence → low
...
- [x] **步骤 4：验证 unknown 不会单独升高**

增加参数化测试：`unknown + 缺失证据` 为低优先级，`unknown + 索引/从略说明` 为中优先级，`unknown + 一般质量警告` 为中优先级；叠加证据失效、冲突或严重质量异常后才为高优先级。所有场景继续保留披露结论和适用性状态，低优先级不得被解释为已披露或已确认不适用。

- [x] **步骤 5：运行分类器测试确认 GREEN**
...
首次模拟得到高 12、中 530、低 35，用户认为中优先级失去筛选作用。2026-07-18 已确认采用 `risk-v2.1`：`unknown + 无证据` 和 `partial + 有效直接证据` 下调为低优先级，`unknown + 仅索引/从略说明` 保持中优先级，适用性待判定作为独立工作队列。最终只读模拟确认高 12、中 60、低 505，适用性待判定 343，且数据库写入前后完全一致。
...
applicability_status VARCHAR(32) NULL
```

在 `review_snapshots` 增加人工适用性判断：

```text
reviewed_applicability_status VARCHAR(32) NULL
```

历史 risk-v1 行保持 `NULL`，避免将未知历史状态错误标记为 `missing` 或 `undetermined`。新 risk-v2 风险快照由应用服务强制写入非空的证据状态和适用性状态。迁移只增加字段和必要索引，不删除旧列，不修改 `risk_rule_version='risk-v1'` 的历史行。

- [x] **步骤 3：新增 API 字段并保留兼容别名**
...
- [x] **步骤 1：将当前规则版本集中为 risk-v2**

新 run 和部署后的 retry 使用 `risk-v2`；父 run 的历史快照继续保留其原规则版本。每次风险计算必须把 `risk_rule_version` 写入追加式快照。

- [x] **步骤 2：分析流程调用新分类器**

从评估结果构造明确上下文，禁止 `risk_service` 根据单一 verdict 隐式推断全部证据和适用性状态。已有 assessment 的明确异常产生高优先级和可解释原因码；没有 assessment 的分析失败在 run 维度单独统计为等同高优先级的阻断项，不伪造 assessment。

- [x] **步骤 3：review 后重新计算优先级**
...
正式输出阻塞分析失败/结果不完整和未解决高优先级 assessment。输出元数据记录：
...
数据库中的原始 engine/audit 值不修改。接口边界明确如下：

- `/api/reports/{report_id}/assessments/{assessment_id}` 新增 `rationale_display` 和 `missing_items_display`，前端使用中文展示字段；原 `rationale` 和 `missing_items` 保持原始值；
- `/api/runs/{run_id}/assessments` 等旧接口继续返回原始契约，不做静默替换；
- CSV/JSON 和版本化输出新增 `rationale_zh`、`missing_items_zh`，保留原始列供审计；
- 已有数据库记录通过读取时映射直接获得中文展示，无需重新分析或回写。
...
## 十四、任务 12：隔离演示库完整人工验收

**前置条件：** 自动门禁全部通过，用户已确认保留历史并通过重复上传创建新报告；reset 只作为维护能力，不是本轮验收前提。

- [x] **步骤 1：确认环境边界**

打印并人工确认数据库名、环境类型、上传目录和运行目录。目标必须为演示环境；若识别为当前保留库或环境标识不明确，立即停止。

- [x] **步骤 2：确认本轮不执行 reset**

确认保留演示库历史记录，通过 `duplicate_policy=create_new` 创建新的报告实例。未删除原始报告、标准资产、旧复核、整改任务或输出版本。

- [x] **步骤 3：重复上传并创建新报告分析**

使用远景 2024 中文 ESG 报告，从重复提示、明确重新上传、metadata 确认、分析、完整核查、复核、整改任务到版本化输出完整走一遍。

- [x] **步骤 4：使用普通 Chrome 验收**
...
2. `unknown + 无证据` 为低优先级，`unknown + 仅索引/从略说明` 为中优先级，`unknown` 不再天然进入高优先级；
...
8. 正式输出阻塞分析不完整和未解决高优先级，同时披露未解决中优先级和适用性范围；
