...
- 迁移停止点：剩余 per-ID explicit verdict 为 68 条，全部是 `unknown + needs_manual_review`，且 `allowed_pages` / `candidate_pages` 均为空。这些 contract 当前承担无证据 guardrail，用于清空容易误命中的候选页或泛化证据；继续迁移会削弱证据过滤边界。剩余清单输出为 `tmp/review/remaining_explicit_verdicts.csv`。
- ontology 后 577 回归产物：`tmp/review/current_577_review_after_ontology.csv`、`tmp/review/current_577_review_after_ontology_audit.json`、`tmp/review/current_577_review_ontology_diff.csv`、`tmp/review/current_577_review_ontology_diff_summary.json`。最新 gate 结论为 audit 通过，`compilation_overlap=0`，`first_pass_quality` 的 disclosed/partial/unknown delta 均为 0。
