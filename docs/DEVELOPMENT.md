...
- 根据用户主观反馈完成背景图片清晰度修正：桌面侧边栏 opacity 从 0.40 提高到 0.50，工作台首次使用、已有报告和报告总览主图分别调整为 0.28、0.25 和 0.23。
- 轻度档仍偏浅后，经用户批准删除侧边栏白色渐变与绿色光晕、工作台两种状态和报告总览线性渐变，共删除 5 个遮罩元素；图片资产、定位、滤镜、动效和布局保持不变。
- 视觉参数采用测试先行：两轮目标测试均先观察到 4 个预期失败，最小修改后 3 个目标测试文件、9 项测试通过。
- 桌面浏览器复验覆盖首页和报告总览；390px 窄屏仍隐藏桌面侧边栏并显示移动导航。标题、按钮、`577`、导航和状态徽章保持可读。
- 完整前端门禁为 39 个测试文件、150 项测试通过，typecheck 和 production build 通过；lint 保持 0 error、2 个既有 warning。demo health 为 `status=ok`、`app_env=demo`，首页返回 200。
- 本轮形成 `v1.3.1` 后的前端视觉补丁基线，没有修改后端、API、数据库、GRI 口径、AI/OCR/RAG、人工复核或导出语义；正式版本仍保持 `v1.3.1`。
- 新增共享 `ReportPageHero`，报告总览、完整核查、人工复核、整改任务、输出版本和审计时间线统一使用紧凑主图头部。完整核查/输出版本复用披露文档图，人工复核使用多维比较图，整改任务/审计时间线使用监控图；五个业务页保持静态，报告总览保留 Ken Burns。
- 主图继续使用 opacity 0.23 且不增加遮罩。浏览器发现首屏主图懒加载告警后，以测试先行补充 `loading="eager"`；复验六页均为 eager 加载，模块主图浏览器告警为 0。
...
- 统一主图已纳入 `v1.3.1` 后续前端冻结基线；没有修改后端、API、数据库、GRI、AI/OCR/RAG、人工复核或导出语义，正式版本号与 tag 仍保持 `v1.3.1`。
...
- 两项补丁仅涉及前端展示、测试和文档，没有写入人工快照、修改历史 run 或调用外部模型/OCR；用户已确认本轮验收收尾，当前视觉与 OCR 展示基线冻结。
