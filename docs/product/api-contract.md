...
查询：`page`、`page_size`、`status`、`year`、`search`、`sort`。返回分页 `ReportSummary`。
...
返回报告详情、自动识别 metadata、确认状态和最新 run。
...
幂等：相同值重复确认返回当前报告；修改已确认值生成新审计事件。

### `POST /api/reports/{report_id}/analyze`

请求保留 `confirm_llm`、`enable_ocr`、`ocr_pages`。报告 metadata 未确认返回 409。同一报告存在 running run 时返回 409。
...
返回报告摘要、结论分布、风险分布、高风险复核进度、GRI 主题分布、失败项数、整改摘要和最新输出。

### `GET /api/reports/{report_id}/assessments`

查询：`page`、`page_size`、`gri_topic`、`risk_level`、`system_verdict`、`effective_verdict`、`review_status`、`evidence_status`、`action_status`、`search`、`sort`。
...
支持状态、优先级、GRI 主题和 requirement 筛选。
...
允许修改状态、负责人、截止日期和完成说明。完成、取消、重开必须填写备注。
...
请求格式列表：`assessment_xlsx`、`management_pdf`、`actions_xlsx`、`print_html`。允许任意复核进度，输出带草稿标识。
...
- 兼容窗口持续两个连续切片验收周期。每个周期都必须执行旧接口自动回归、替代接口契约测试和 OpenAPI 类型生成。
