...
`can_start_new_demo=true` 仅表示后端配置和实际数据库连接均通过 demo 安全校验；普通 `esg_agent` 环境必须返回 `false`。

前端提供“查看已有结果”和“开始新演示”两条路径；后者必须二次确认、先完成 demo reset，再自动重新上传当前文件。
...
仅 `APP_ENV=demo`、配置数据库名和 `SELECT current_database()` 均为 `esg_agent_demo`、且运行时目录通过边界校验时可用。请求必须由前端二次确认触发：
