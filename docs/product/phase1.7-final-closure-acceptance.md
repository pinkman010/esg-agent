...
截至 2026-07-29，Phase 1.7 的代码、契约、双报告工程验证、干净环境验证和 Chrome 自动验收均已完成。提交 `876859f` 已形成正式 `v1.2` 发布基线，`main` 与远端一致：

> esg-agent 已形成可本地运行的单报告 ESG 披露分析闭环，支持数字文本型 PDF 的 GRI 577 项结构化核查、证据定位、规则结果、AI 辅助建议、人工复核快照、整改跟踪、草稿/正式输出、审计追踪和失败恢复。

正式发布基线为 `v1.2`（`876859f`）。本报告只证明本地产品闭环和工程质量，不构成 GRI 认证、ESG 专家结论、法律意见、外部鉴证或企业级生产部署承诺。

> 发布后注记（2026-07-29）：v1.2 发布后发现普通产品与长期回归数据环境混用、浏览器仍连接旧后端、复核人跨报告回填及主流程开发字段名暴露。上述问题由 v1.2.1 补丁候选处理；本注记不改写 Phase 1.7 当时的门禁、Chrome 证据或 v1.2 发布结论。详见 `docs/product/v1.2.1-post-release-remediation-acceptance.md`。
...
Phase 1.7 完成后不自动创建 Phase 1.8。独立复核、用户批准后的分支集成、`v1.2` 标签和推送均已完成；后续进入真实使用、问题收集和条件化迭代阶段。
