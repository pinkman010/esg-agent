...
ESG-Agent 第一版面向企业 ESG 团队，提供单报告 GRI 核查闭环。系统对每份报告分析全部 577 条 eligible GRI requirement，保留跨企业、跨报告格式的识别泛化能力，并通过复核优先级与适用性两个独立队列组织人工工作。
...
5. **范围诚实：** “高优先级项目已复核”不表示全部 577 条已人工确认。
...
截至 2026-07-18，代码迁移 head 为 `0010_risk_v2_dimensions`。报告上传与 metadata 确认、577 条分析、工作量加权阶段进度、active run 门禁、后台任务恢复、risk-v2.1 四维模型、高优先级与适用性双队列、追加式复核、三栏工作台、整改任务创建与状态更新、版本化输出和 demo 在线重置均已有实现。当前 Envision 577 只读模拟基线为高 12、中 60、低 505，适用性待判定 343；这些数量只用于本次固定输入回归，不进入分类规则。Envision regeneration gate 和 Goldwind 100 条人工 gold gate 继续独立验证分析事实。

本地 MVP 人工产品验收已经完成：隔离 demo 中重复上传创建新报告、metadata、七阶段分析、577 条分页、双队列、三栏 PDF、整改任务、草稿和正式输出门禁均已通过。后端 555 项测试、前端 19 个测试文件 51 项测试、typecheck、production build 和 Envision 577 零 verdict 回归通过。适用性当前页批量确认已经实现；通用 verdict 批量复核、独立 reopen API、按 report 查询的审计 API、单个 export metadata/文件下载 API，以及完整整改任务清单导出仍为增强项。实际接口状态以 OpenAPI 和 `docs/product/api-contract.md` 的状态标记为准。
