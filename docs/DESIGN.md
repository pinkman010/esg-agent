...
范围投影使用同一报告的运行谱系有效视图：从最新 run 沿 `parent_run_id` 向上遍历，同一 requirement 取最近 assessment，并返回 `source_run_id`。明确失败但没有 assessment 的独立项标为 `failed`，从未生成的独立项标为 `not_generated`；两类行都不得伪造 verdict、evidence、risk、applicability 或 review 状态。上下文项继续保持 `context_incorporated`。遍历执行报告一致性校验、循环检测和最大深度限制，历史 run 只读，不复制或覆盖。
...
截至 2026-08-23，Phase 1.7 已完成最终闭环与发布就绪验收；v1.2 后的产品缺陷修复、前端视觉与中文展示、AI 候选路由治理和 OCR 单页受控试点已通过完整门禁并纳入发布基线，侧边栏视口裁切修复进一步形成正式 `v1.3.1` 补丁基线。代码迁移 head、main 数据库和 demo 数据库均为 `0012_chunk_embeddings`；本轮没有 migration 或表结构变化。结构 manifest 为 `gri-requirement-checklist-v3`；产品方法版本为 `envision-method-v1.1`；结果裁决版本为 `envision-result-v1.1`；复核优先级规则为 `risk-v2.1`。

v3 技术结构为 `577/499/78/0`。部分失败、失败项 retry 和服务重启恢复统一通过运行谱系有效视图形成当前产品状态；范围表、dashboard、review、export gate 和 report status 读取同一语义。失败或未生成项独立展示分析状态，不能形成虚假正式结论。dashboard 分别返回失败数、未生成数和聚合分析不完整数；旧失败数字段不扩大语义。Envision v3 gate 的 global fallback、新增 false disclosed 和新增 wrong source page 均为 0，audit 为 0 error、0 warning。后端 823 项测试和 Ruff、前端 39 个测试文件 150 项测试、typecheck 和 production build 全部通过。该历史正式基线的后端包、前端包和 OpenAPI `info.version` 统一为 `1.3.1`；当前运行时 OpenAPI 的 40 条 `/api/*` 路径已全部同步到前端生成类型。

1.5 可复现交付候选在不改变产品语义的前提下，将后端包、前端包和 OpenAPI `info.version` 同步为 `1.5.0`，公开交付名保持 `1.5`。最近正式发布 tag 仍为 `v1.3.1`；等价干净环境验收和用户确认完成前，不创建 `v1.5` tag 或 Release。
...
通用 verdict 批量复核、生产 OCR、RAG Phase 2 和企业级平台能力进入条件化路线图。旧 `review_decisions` 和旧 API 继续处于兼容窗口。完整结果、限制和触发条件见 `docs/product/phase1.7-final-closure-acceptance.md`、`docs/product/v1.3-final-release-acceptance.md`、`docs/product/v1.3.1-final-release-acceptance.md`。
