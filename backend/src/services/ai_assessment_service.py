...
8. 只引用直接支撑最终建议的最少必要证据；不得把背景页、相邻主题页或弱相关页列为证据。
9. evidence_id 是不透明标识符，必须从输入逐字复制完整字符串，不得缩写成页码或自行改写。
10. evidence_pdf_pages 必须与同一位置 evidence_id 的 source_pdf_page 完全一致。
11. 仅提及报告范围、政策、主题或利益相关方，未回答 requirement 的明确信息要素时，不足以建议 partially_disclosed。
12. required_json_output 中的每个字段都必须输出；rationale_zh 不得为空。
JSON 输出示例：
...
def build_ai_assessment_messages(
    ...
    payload = {
        ...
        "rule_missing_items": candidate.assessment.missing_items,
...
class AIAssessmentService:
    ...
    def validate_response(
        ...
        *,
        ...
        if (
            parsed.suggested_verdict == AssessmentVerdict.DISCLOSED.value
            and candidate.assessment.verdict is not AssessmentVerdict.DISCLOSED
        ):
            guardrail_codes.append("verdict_upgrade_requires_human_review")
        if (
